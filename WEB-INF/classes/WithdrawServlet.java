...
out.println("Amount to be withdrawn: " + amount);
if (pwd.isEmpty()) {
    out.println("<html><body><h3>Please enter your password to confirm withdraw</h3></body></html>");
    return;
}

PersonDAO personDAO = new PersonDAO(); 
PersonInfo person = personDAO.Withdraw(pwd,amount,sessionusername); 
out.println("<h1>Withdrawing Info:</h1>");

	if (person != null){ 
		out.println("<h3>"+ person.toString() +"</h3>" ); 
		out.println("<h3>Withdraw Successfull</h3>" ); 
			} 
	else{ 
		out.println("<h3>Error doing Withdraw</h3>" ); 
	} 
	out.println("</body>"); 
	out.println("</html>");
	out.close(); 
